...
scheduled to port there (#129). Both repositories describe a mission broader
than that name, so the board's name can widen or change as the app does;
revisit it when it stops matching what the two build together. Until then the
mismatch is a known intermediate state rather than a sign you are on the wrong
board.
...
configuration or pre-commit output. The legacy simulator is protected against
new lint regressions by a ratchet gate (`python
scripts/check_legacy_pylint_ratchet.py`) that fails in pre-push and CI if
`simulate_cribbage_games.py` reports any message beyond the committed baseline
in `.pylint-legacy-baseline.json` (comparing path, line, and symbol). Plain
`pylint simulate_cribbage_games.py` remains advisory until notices are cleared
under #37; to refresh the baseline after an approved tooling bump, run
`python scripts/check_legacy_pylint_ratchet.py --refresh` and state the
rationale in the pull request description. The legacy simulator is also gated on
duplicate-code similarities (`pylint --persistent=n --disable=all
--enable=duplicate-code simulate_cribbage_games.py`). The artifact pipeline is
covered by local pre-commit and CI checks for both `pylint --persistent=n
artifact_pipeline` and `flake8 artifact_pipeline`. The current pylint
configuration does not enforce magic-number checks or unusually strict
short-variable-name checks. Use `npm run spellcheck` to catch misspellings and
many opaque identifier fragments that pylint accepts as valid snake_case.
